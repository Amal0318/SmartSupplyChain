...
import { generateAiReportApi } from '../api/ai_reports';
...
export default function App() {
  ...
  const handleLoadSampleData = (domain: RoleType) => {
    ...
    setActiveTab('upload');
    handleDatasetUploaded(newDataset);
  };
  ...
  const handleRedirectToInsights = async () => {
    try {
      await generateAiReportApi();
    } catch (err) {
      console.warn('Backend report generation failed, using existing:', err);
    }
            ...
            {activeTab === 'upload' && currentRole !== 'master' && (
              <div>
                <DataUploadSection
                  ...
                  onRedirectToInsights={handleRedirectToInsights}
                />
